<template>
    <div>
        <div class="times">
            <span :class="type == 1 ? 'check' : ''" @click="selectType(1)">{{ $t('gold_market_info_chart.text1') }}</span>
            <span :class="type == 2 ? 'check' : ''" @click="selectType(2)">{{ $t('gold_market_info_chart.text2') }}</span>
            <span :class="type == 3 ? 'check' : ''" @click="selectType(3)">{{ $t('gold_market_info_chart.text3') }}</span>
            <span :class="type == 4 ? 'check' : ''" @click="selectType(4)">{{ $t('gold_market_info_chart.text4') }}</span>
            <span :class="type == 5 ? 'check' : ''" @click="selectType(5)">{{ $t('gold_market_info_chart.text5') }}</span>
            <span :class="type == 6 ? 'check' : ''" @click="selectType(6)">{{ $t('gold_market_info_chart.text6') }}</span>
            <span :class="type == 7 ? 'check' : ''" @click="selectType(7)">{{ $t('gold_market_info_chart.text7') }}</span>
        </div>
...
import { index_gold_xintiao1_api, index_gold_xintiao_api } from "@/assets/js/api";
...
export default {
    ...
    methods: {
        ...
        selectType(type) {
            ...
            switch (type) {
                case 1:
                    ...
                    break;
                case 2:
                    this.getKlineDate('5min');
                    break;
                case 3:
                    this.getKlineDate('15min');
                    //this.fnGetDemoChart(this.calculateMADay(15, this.ldata.data));
                    break;
                case 4:
                    this.getKlineDate('30min');
                    break;
                case 5:
                    this.getKlineDate1('day');
                    break;
                case 6:
                    this.getKlineDate1('week');
                 
                    break;
                case 7:
                    this.getKlineDate1('month');
        ...
        getKlineDate(min) {
            ...
            index_gold_xintiao1_api({
                symbol: this.symbol,
                min: min,
            }).then(res => {
                if (res.code == 200) {
                    this.ldata = res;
                    this.fnGetDemoChart(this.calculateMADay(1, this.ldata.data));
                }
            })
        },
        getKlineDate1(min) {

            index_gold_xintiao_api({
                symbol: this.symbol,
                min: min,
            }).then(res => {
                if (res.code == 200) {
                    ...
                    xAxis: [
                        {
                            ...
                            axisLabel: {
                                rotate: 45, // 旋转角度
                                interval: 10, //加入axisLabel字段,interval后面加你想要间隔的个数
                            }
                        },
                        {
                            type: 'category',
